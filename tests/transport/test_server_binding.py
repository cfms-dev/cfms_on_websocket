...
def _set_trusted_proxy_networks(monkeypatch, client_address, values) -> None:
    monkeypatch.setattr(
        client_address,
        "global_config",
...
def test_ipv6_address_preserves_dualstack_setting(
    client_address, dualstack_ipv6: bool, expected_dualstack_ipv6: bool
):
    family, actual_dualstack_ipv6 = client_address.get_bind_options(
        "::1", dualstack_ipv6
    )
...
def test_hostname_uses_system_resolver_family(monkeypatch, client_address, family):
    ...
    actual_family, dualstack_ipv6 = client_address.get_bind_options(
...
def test_real_ip_from_trusted_peer_is_validated(
    monkeypatch, client_address, real_ip, expected
):
    _set_trusted_proxy_networks(monkeypatch, client_address, ["10.0.0.0/8"])
    websocket = _websocket("10.0.0.2", {"X-Real-IP": real_ip})
...
def test_ipv4_mapped_forwarded_address_uses_ipv4_trust_rules(
    ...
    _set_trusted_proxy_networks(
        monkeypatch, client_address, ["10.0.0.0/8", "192.0.2.0/24"]
    )
    websocket = _websocket(
        "10.0.0.2",
